...
function addIdea(event) {
  var unscramblerChars = "/)#23%1&60!()"

  var randQuote = Math.round(Math.random() * (ideas.length - 1));
  console.log(ideas[randQuote][0] + "\n" + ideas[randQuote][1]);

  var sTop = replaceWithRandom(ideas[randQuote][0], unscramblerChars);
  var sBot = replaceWithRandom(ideas[randQuote][1], unscramblerChars);

  document.getElementById("mainText").textContent = sTop;
  document.getElementById("subText").textContent = sBot;

  console.log(sTop);
  console.log(sBot);

  var topA = randOrder(sTop.length);
  var botA = randOrder(sBot.length);


  replaceString(topA, sTop, ideas[randQuote][0], 0, 'mainText', 1);
  replaceString(botA, sBot, ideas[randQuote][1], 0, "subText", 4);
  //replaceAtRecursion(sTop, topA, 0, ideas[randQuote][0], sTop.length, 1);

}

function replaceWithRandom(string, replacementChars) {
  var newString = "";

  for (var i = 0; i < string.length; i++) {
    var randSpot = Math.round(Math.random() * (replacementChars.length - 1));

    var char = replacementChars.charAt(randSpot);
    newString += char;
  }
  return newString;
}
...
function replaceAtRecursion(string, indexArray, n, replace, loops, number){
  if(n >= indexArray.length) return string;
 var newString = replaceAt(string, indexArray[n], replace.charAt(indexArray[n]));

 console.log(newString);

 if(number <= loops) return replaceAtRecursion(newString, indexArray, n + 1, replace, loops, number + 1);
 else return string;

}


function replaceString(orderArray, origString, newString, n, id, charPerLoop) {

  console.log(origString);
  var betweenString = replaceAtRecursion(origString, orderArray, n, newString, charPerLoop, 1);//replaceAt(origString, orderArray[n], newString.charAt(orderArray[n]));

  document.getElementById(id).textContent = betweenString;

  n += charPerLoop;//charPerLoop;
  sleep(1);

  console.log(orderArray.length + " > " + n);

  if(orderArray.length > n) {

    setTimeout(function(){
      replaceString(orderArray, betweenString, newString, n, id, charPerLoop);
    }, 50);
  }
  else {
    return;
...
function randOrder (total) //Fisher-Yates Shuffle
{
  var array = new Array(total)
  var rArray = new Array(total);
  ...
  for(var i = 0; i <= array.length;) {
    var rand = Math.round(array.length*Math.random());

    if(rArray[rand] == null || rArray == undefined) {
      rArray[rand] = i;
      i++;
    }
...
//var unbor = document.getElementById("unborify");
document.addEventListener('click', addIdea);
//console.log(randOrder(4));
